...
SELECT DISTINCT(last_names) FROM customer;
8. Select all the distinct first names from the actor table
 SELECT DISTINCT(first_names) FROM actor; 
...
SELECT DISTINCT first_names FROM actor;
13. Select rental_id and the difference between return_date and rental_date in rental table SELECT rental_id,return_date-rental_date FROM rental;