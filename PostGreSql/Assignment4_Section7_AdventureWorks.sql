-- 1.Return the name,weight and productnumber of all the products ordered by weight from 
-- lightest to heaviest.(Remember to use schema to reach table.It is in production schema.)

SELECT name,weight,productnumber FROM production.product ORDER BY weight ASC

--2.Return the records from productvendor for productid = 407 in order of averageleadtime 
-- from shortest to longest. (You'll have to figure out which schema this is in.)

SELECT * FROM purchasing.productvendor WHERE productid=407
ORDER BY averageleadtime ASC

-- 3.Find all the salesorderdetail records for productid 799 and order them by 
-- largest orderqty to smallest.
